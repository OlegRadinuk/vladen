...
echo "4. ОБЯЗАТЕЛЬНО: cp -r .next/static .next/standalone/.next/static && cp -r public .next/standalone/public"
echo "5. pm2 start .next/standalone/server.js --name vladen"
echo "6. pm2 save && pm2 startup"
echo "7. Скопируй deploy/nginx.conf в /etc/nginx/sites-available/$DOMAIN"
echo "8. ln -s /etc/nginx/sites-available/$DOMAIN /etc/nginx/sites-enabled/"
echo "9. certbot --nginx -d $DOMAIN -d www.$DOMAIN"
echo "10. nginx -t && systemctl reload nginx"
